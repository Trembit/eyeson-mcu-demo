...
`;
            ...
            return new Promise((resolve, reject) => {
                try {
                    const janus = new Janus({
                        server: 'wss://janus-raw-new-wss.trembit.com',
        ...
        const streamingPlugin = (cameraId) => {
            return new Promise(async (resolve, reject) => {
                const handler = {
                    handler: null,
                ...
                const janusLibInstance = await createSession();
                console.log('!!!!!!!!!!!!!!!', janusLibInstance);
                janusLibInstance.attach({
                    plugin: 'janus.plugin.streaming',
                    success: async (plugin) => {
                        console.log('JS: janus.attach() success plugin=', plugin);
                        handler.handler = plugin;
                        resolve(handler);
                        await plugin.send({message: {request: 'watch', id: cameraId}});
                    },
                    error: (reason) => {
                        console.log('JS: janus.attach() error reason=', reason);
                        reject(reason);
                    },
                    ondata: (data) => {
                    ...
                    onmessage: async (message, jsep) => {
                        if (!!jsep) {
                            handler.handler.createAnswer({
                                ...
                                success(jsep) {
                                    ...
                                    handler.handler.send({message: body, jsep});
                                },
                                error(error) {
                                    Janus.error('WebRTC error:', error);
                        ...
                        handler.messages$.next({message, jsep});
                    },
                    onlocalstream: async (stream) => {
                        // await this.turnHighQuality(stream, !!this. auth.getPinnedAt(this.room.id as string));
                        handler.streams$.next(stream);
                    },
        ...
        const startRtsp = async () => {
            try {
                const janusClient = new janus.Client('wss://janus-raw-new-wss.trembit.com', {
                    iceServers: [{urls: 'stun:stun.l.google.com:19302'}],
                    keepalive: 'true',
                });
                ...
                const janusLibInstance = await streamingPlugin(id);
        ...
        const startMeeting = () => {
            eyeson.onEvent(event => {
                ...
                if (type === 'accept') {
                    ...
                    console.error(`Exit meeting due to error "${event.name}"`);
                }
                else if (type === 'exit') {
                    _meeting = false;
